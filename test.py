corrects = [7, 2, 1, 0, 4, 1, 4, 9, 5, 9, 0, 6, 9, 0, 1, 5, 9, 7, 3, 4, 9, 6,
...
corrects = corrects[:100]

print("Enter address of your output file: ")
output_address = input()
output_file = open(output_address, "r")
our_output = output_file.read()

list = our_output.split('\n')

results = []

for item in list:
    results.append(int(item.split()[6]))

mistake_count = 0
for x, y in zip(corrects, results):
    if x != y:
        mistake_count += 1

accuracy_percent = 100 - (mistake_count / len(results)) * 100

print("Number of mistakes:", mistake_count)
